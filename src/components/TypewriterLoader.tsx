...
export default function TypewriterLoader() {
  ...
  const checkAllReady = useCallback(
    (imagesLoaded: number) => {
if (imagesLoaded >= CARD_IMAGES.length) {
      setIsExiting(true);
  ...
  useEffect(() => {
    ...
    CARD_IMAGES.forEach((src) => {
      ...
      img.onload = () => {
        imagesLoaded += 1;
        setLoadedCount(imagesLoaded);
        checkAllReady(imagesLoaded);
      };
      img.onerror = () => {
        imagesLoaded += 1;
        setLoadedCount(imagesLoaded);
    ...
    const handleVideoReady = () => {
      checkAllReady(loadedCount);
    };

    window.addEventListener("kumo-video-ready", handleVideoReady);
    ...
    return () => {
      window.removeEventListener("kumo-video-ready", handleVideoReady);
  ...
  return (
    <>
      <style>{`
        ...
        .typewriter-loader.exiting {
          opacity: 0;
          transform: scale(0.92) translateY(20px);
          filter: blur(8px);
        }

        .typewriter-loader .typewriter {
          position: relative;
      ...
      <div className={`typewriter-loader ${isExiting ? "exiting" : ""}`}>
